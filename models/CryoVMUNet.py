...
class CryoVMUNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=3, layer_scale_init_value=1e-6, H_SS2D=H_SS2D, block=Block, pretrained=None,
        ...
        self.aspp = ASPP(in_channels=c_list[5], atrous_rates=[6, 12, 18], out_channels=c_list[5])
...
class Local_SS2D(nn.Module):
    ...
    def forward(self, x):
        ...
        offset = self.offset_conv(x1)  # [B, 2*3*3*(C//2), H, W]
        
        x1 = self.dw(x1, offset)       # [B, C//2, H, W]
        
        B, C, a, b = x2.shape
        x2 = x2.permute(0, 2, 3, 1)    # [B, H, W, C//2]
        x2 = self.SS2D(x2)             # [B, H, W, C//2]
        x2 = x2.permute(0, 3, 1, 2)    # [B, C//2, H, W]
